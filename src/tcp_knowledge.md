...
1. **TCP 可靠性保障：**
    * TCP 是如何保证数据传输的可靠性、有序性和完整性的？请至少从序列号、确认应答、超时重传和校验和四个方面进行详细阐述。
    * 解释 TCP 的**累计确认**机制。它与选择性确认 (SACK) 有何区别和联系？为什么需要 SACK？
...
以上就是一份针对高级 Web 后端工程师的 TCP 面试题清单。这些问题旨在考察你对 TCP/IP 协议的全面理解，以及将理论知识应用于实际系统设计和性能优化的能力。

接下来，我们将就**每道题的细节**进行深入讨论。你准备好了吗？
